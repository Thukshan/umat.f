...
C
	  ...
	  STATEV(115)=STATEV(112)
	  
	  DO i=1,nSys
          gammaslip2(i) =  STATEV(116+i) 
      END DO
	  
	  STATEV(129)=STATEV(122)
...
C     *** USE NEWTON METHOD TO DETERMINE STRESS INCREMENT ***
     ...
     + xfreq, gammazero, xboltzman, xtemp, rhoinitial, gammaslip, gammadot)
...
C     *** UPDATE OUTPUT VARIABLES ***
	 ...
	 DO I=1,nSys
	    gammaslip2(I) = gammaslip2(I) + gammadot(I)*dtime
	 END DO
     ...
     + npt, kinc
      ...
      gmatinv =  gmatinvnew            
       ...
       if (maxval(gmatinv) > 1) then
          write(*,*) "something very wrong with gmatinv",maxval(gmatinv)
C          call XIT
       end if
...
C     Combine SSDs and GNDs      
      ...
      STATEV(92) = Gcycle
      STATEV(93) = cyc
      ! slip on individual slip systems at each time increment
      DO i=1,nSys
